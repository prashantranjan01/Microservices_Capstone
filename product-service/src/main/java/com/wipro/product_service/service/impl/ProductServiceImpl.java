...
import com.wipro.product_service.exception.*;
...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public ProductResponse createProduct(ProductRequest request, HttpServletRequest servletRequest) {
        String authHeader = servletRequest.getHeader("Authorization");
        System.out.println(authHeader);

        if (!permissionService.hasPermission(authHeader, "CREATE_PRODUCT")) {
            throw new PermissionDeniedException("Insufficient permissions");
        }
        ...
        product.setStatus(request.getStockQuantity() > 0 ? ProductStatus.ACTIVE : ProductStatus.OUT_OF_STOCK);
